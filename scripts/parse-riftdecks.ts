...
function parseDeckMarkdown(md: string, sourceUrl: string): DeckJson | null {
  ...
  return {
    ...
    champion,
    player,
    tournament: "S3 Xi'an Regional Open",
    date: "2026-05-24",
    placement,
    playerCount: 640,
    set: "Unleashed",
...
// Main
const rawDir = path.join(__dirname, "../data/raw-scrapes/s3-xian-regional-open");
...
for (const file of files) {
  ...
  const fileName = `s3-xian-${deck.placement ?? "unranked"}-${playerSlug}.json`;
