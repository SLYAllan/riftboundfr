...
const rawDir = path.join(__dirname, "../data/raw-scrapes/houston-rq");
...
console.log(`Parsing ${files.length} Houston RQ deck files...`);
...
for (const file of files) {
  ...
  const fileName = `houston-rq-${deck.placement ?? "unranked"}-${playerSlug}.json`;
