$urlFile = Join-Path $PSScriptRoot "..\data\raw-scrapes\las-vegas-rq-urls.txt"
$outDir = Join-Path $PSScriptRoot "..\data\raw-scrapes\las-vegas-rq"
$urls = Get-Content $urlFile | Where-Object { $_ -match '\S' }

Write-Host "Las Vegas RQ: scraping $($urls.Count) decklists..."
$done = 0; $fail = 0
