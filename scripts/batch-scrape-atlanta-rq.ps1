$urlFile = Join-Path $PSScriptRoot "..\data\raw-scrapes\atlanta-rq-urls.txt"
$outDir = Join-Path $PSScriptRoot "..\data\raw-scrapes\atlanta-rq"
$urls = Get-Content $urlFile | Where-Object { $_ -match '\S' }

Write-Host "Atlanta RQ: scraping $($urls.Count) decklists..."
$done = 0; $fail = 0
