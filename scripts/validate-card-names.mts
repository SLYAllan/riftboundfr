...
const add = (v: string | null) => {
  ...
  if (n && !realSet.has(n)) { realSet.add(n); realArr.push(n); }
};
for (const c of cards) { add(c.name); add(c.cleanName); }
...
try {
  ...
  for (const m of ban.matchAll(/"([^"]+)"/g)) add(m[1]);
} catch { /* optionnel */ }
...
function suggest(cand: string): string | null {
  ...
  // suggestion utile seulement si proche
  return best && bd <= Math.max(2, Math.floor(c.length * 0.34)) ? best : null;
}
...
for (const f of files) {
  ...
  for (const c of cands) {
    ...
    const s = suggest(c);
    suspects.push(s ? `${c}  →  « ${s} » ?` : `${c}  (aucune carte proche)`);
  }
  if (suspects.length) {
    totalSuspects += suspects.length;
    console.log(`\n### ${f} — ${suspects.length} suspect(s)`);
...
console.log(`\n${totalSuspects === 0 ? "✅ Tout est clean." : `⚠️ ${totalSuspects} suspect(s) à vérifier (Whisper ?).`}`);
