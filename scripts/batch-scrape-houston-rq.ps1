$urlFile = Join-Path $PSScriptRoot "..\data\raw-scrapes\houston-rq-urls.txt"
$outDir = Join-Path $PSScriptRoot "..\data\raw-scrapes\houston-rq"
$urls = Get-Content $urlFile | Where-Object { $_ -match '\S' }

Write-Host "Houston RQ: scraping $($urls.Count) decklists..."
$done = 0; $fail = 0
