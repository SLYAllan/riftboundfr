$urlFile = Join-Path $PSScriptRoot "..\data\raw-scrapes\lille-rq-urls.txt"
$outDir = Join-Path $PSScriptRoot "..\data\raw-scrapes\lille-rq"
$urls = Get-Content $urlFile | Where-Object { $_ -match '\S' }

Write-Host "Lille RQ: scraping $($urls.Count) decklists..."
$done = 0; $fail = 0
