...
export function OverlayDashboard({ token, cleCompagnon, initial }: { token: string; cleCompagnon: string; initial: OverlayStateData }) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
      ...
      <section className="rounded-xl border border-hairline bg-surface p-4">
        <h2 className="text-sm font-semibold">{t("Installer votre stream mobile")}</h2>
        <ol className="mt-2 space-y-1.5 text-xs text-ink-secondary">
          <li><strong className="text-ink">1.</strong> {t("Copiez l’overlay mobile dans Moblin.")}</li>
          <li><strong className="text-ink">2.</strong> {t("Ouvrez le compagnon sur le second appareil.")}</li>
          <li><strong className="text-ink">3.</strong> {t("Remplissez les joueurs, puis lancez la partie.")}</li>
        </ol>
        <a href={lien("/articles/streamer-riftbound-avec-un-telephone")} className="mt-2 inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-arcane hover:underline">
          {t("Voir le tutoriel complet")}
