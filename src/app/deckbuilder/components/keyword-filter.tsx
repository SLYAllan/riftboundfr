          ...
          <Popover.Popup className="flex max-h-[min(70vh,34rem,var(--available-height))] w-[min(24rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-xl border border-hairline-strong bg-surface p-2 shadow-2xl shadow-black/50 focus:outline-none">
            <div className="relative mb-2">
              ...
              <input
                ...
                aria-label={t("Rechercher une mécanique")}
                placeholder={t("Rechercher une mécanique")}
                className="h-11 w-full rounded-lg border border-hairline bg-canvas pl-9 pr-9 text-sm text-ink placeholder:text-ink-muted focus:border-arcane focus:outline-none"
              />
              {recherche && (
                <button type="button" onClick={() => setRecherche("")} aria-label={t("Effacer la recherche")} className="absolute right-1 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-md text-ink-muted hover:bg-surface-raised hover:text-ink focus-visible:outline-2 focus-visible:outline-arcane">
                  <X size={14} />
                </button>
              )}
            </div>
            ...
            <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto">
              <button type="button" onClick={() => choisir("")} aria-pressed={!value} className="flex min-h-11 w-full items-center rounded-lg px-3 text-left text-sm text-ink-secondary hover:bg-surface-raised hover:text-ink focus-visible:outline-2 focus-visible:outline-arcane">
                <span className="flex-1">{t("Toutes les mécaniques")}</span>
              </button>
              ...
              {CATEGORIES.map((categorie) => {
                ...
                return (
                  <section key={categorie.value} className="mt-2 border-t border-hairline pt-2 first:mt-0 first:border-0 first:pt-0">
                    ...
                    {elements.map((option) => {
                      ...
                      return (
                        <button key={option.value} type="button" onClick={() => choisir(option.value)} aria-pressed={active} className={`flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-left text-sm transition-colors focus-visible:outline-2 focus-visible:outline-arcane ${active ? "bg-surface-raised font-semibold text-arcane" : "text-ink-secondary hover:bg-surface-raised hover:text-ink"}`}>
                          ...
                          <span className="min-w-7 rounded-md bg-canvas px-1.5 py-0.5 text-center text-xs tabular-nums text-ink-muted">{option.count}</span>
                        </button>
                      );
                    })}
                  </section>
              ...
              {visibles.length === 0 && (
                <p className="px-3 py-8 text-center text-sm text-ink-muted">{t("Aucune mécanique disponible")}</p>
              )}
            </div>
          </Popover.Popup>
        </Popover.Positioner>
