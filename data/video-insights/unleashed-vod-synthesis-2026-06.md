# Synthèse VOD compétitives — Riftbound Unleashed (juin 2026)

> **Matière privée, usage interne.** Connaissance extraite de 84 VOD compétitives (RunesAndRift,
> RiftlabTCG) et **reformulée en français** — aucune citation verbatim, jamais de traduction littérale
> (copyright). Ce fichier nourrit META-KNOWLEDGE.md / DECKBUILDING-RULES.md / les fiches. Les noms de
> cartes et de Légendes restent en anglais (noms propres). Les chiffres et avis sont ceux des casters
> (Jibs, Pentastag, Exo/XO…) — à traiter comme **opinion experte**, pas comme donnée DB. **Aucune
> decklist n'est créée à partir de ces VOD** : toute liste vient du scrape réel.
>
> Couverture : RQ Sydney, Vancouver, Utrecht, Hartford, Tianjin, Xi'an + Tournament 15 + guides RiftLab.
> Orthographes Whisper normalisées (Aurelia=Irelia, Massie/Wudu=Master Yi Bladesman, Kazakhs=Kha'Zix,
> Hway/Hui=le moteur de draw de Diana, Volmaw=Ruin Runner-like, Ivan=Ivern).

## 1. Lecture méta générale (set Unleashed, ère Hartford)

- **La consistance prime sur la puissance brute.** Les decks qui montent en day 2 exécutent un plan
  linéaire et répétable (Aurora, Diana, Master Yi, Vex). Beaucoup de listes des bons joueurs convergent.
- **Le violet (Chaos) domine la conversation** : LeBlanc, Aurora, Vex, Diana. Il « punit » de nombreux
  archétypes et pousse les decks de removal pur hors du méta.
- **Le jaune (Ordre) aggro est très bien placé** grâce aux nouvelles pioches (un sort donne « temporary »
  à un recruit et fait piocher 2 ; Sacrifice ; Call the Weak ; board-wipes Ruination).
- **Le de-ramp est l'une des actions les plus punitives du set.** En Aurora, dépenser quoi que ce soit
  avant le tour 3 = quasi défaite.
- **Peu de removal pur dans le méta → Diana prospère** (peu de réponses à ses unités).
- **Différences régionales nettes** : NA (Hartford) joue plus Draven / Kai'Sa / Azir et beaucoup de Set 1 ;
  Europe (Utrecht) joue plus Vex / Lillia / Kha'Zix / Irelia ; Aurora plus présente en Europe.
- **Hartford = dernier event à donner un « Best of Legend » Set 1** → afflux de vieux decks Set 1
  (Ahri, Lux, Yasuo, Annie, et surtout Master Yi) qui gonflent artificiellement certains chiffres.
- **Sydney = premier RQ sans aucun deck > 10 %** : méta jugé sain et diversifié à ce moment.
- **Going first quasi toujours souhaité**, même pour les decks ramp (avantage tempo).

## 2. Tier / lecture par Légende (consensus casters)

> Domaines confirmés conformes à la table canonique riftbound.gg (voir DECKBUILDING-RULES.md). Les VOD
> **confirment** ces paires (lecture par couleur : Rouge=Fureur, Vert=Calme, Bleu=Esprit, Orange=Corps,
> Violet=Chaos, Jaune=Ordre).

**Haut du tableau (S / haut A)**
- **Irelia « Aurelia », Blade Dancer (Calme/Chaos)** — consensus n°1 du format (« queen of Riftbound »).
  Vert+violet = counters verts + disruption violette. Très versatile, beaucoup de cartes en main, forte
  en field large. Gagne Sydney. À Hartford : représentation divisée par deux (effet « dernier Set 1 »).
  Pénalisée par la lourdeur mentale (math de might) et le skill requis. Pièce maîtresse : **Defiant Dance**
  (meilleur signature avec Moonfall), **Stellacorn Herder** (moteur de draw), GA, double counterspell.
- **Master Yi, Wuju Bladesman (Corps/Calme)** — **le plus représenté en day 2**, vainqueur Hartford
  (Factor). Plan linéaire et simple, mais flexible. Core : **Ruin Runner** (« the camel », intuable par
  sorts/abilités — mais meurt à Flurry of Blades qui ne cible pas), **Rengar Trophy Hunter** (ambush
  surprise, carte différenciante), **First Mate** + **Star Spring** (re-stand → double conquer),
  **Punch First / On Guard / Discipline / Challenge** (buffs non-defiables ou assassinat hors combat).
  Tendance : **gear hate en main deck** + **Fiora Peerless** (double sa might en défense 1v1). Piège
  récurrent : on oublie que sa Légende donne **+2 might à une unité qui défend** → trades « gratuits ».
  Pires matchups : Irelia, Diana (contestation early via Gust), decks tier 1 où il n'a plus de « win auto ».

**Tier A**
- **Diana, Scorn of the Moon (Esprit/Chaos)** — **en flèche, vainqueur Vancouver**, 3 en top 8 Hartford.
  Oppressive ce set : mid-range showdown très sortilège-dépendant. Moteur : **Hway/Hui** (draw à chaque
  move, +might via Ride the Wind), **Moonfall** (signature, -2 à tout, le meilleur signature du jeu selon
  le consensus, scaling -4 à -8), **Star-Crossed** + **Fizz** (recyclage), **Vex Apathetic**,
  **Tide Turner** (pull-back Vex pour le 8e point — win-con récurrente). Tech : **Acceptable Losses**
  (anti-gear vs Irelia), **Baron Nashor** side (matchups longs/removal), **Eclipse** (-4 sans plancher).
  **Très faible contre le removal** (rare actuellement) et les decks de sorts de dégâts (Pyke/Ezreal/Jhin).
  Souvent « un shell pour Hway » : les games avec Hway sont radicalement différentes.
- **Ezreal, Prodigal Explorer (Esprit/Chaos)** — **meilleur deck contrôle** du format. Plan long, monte
  jusqu'à 9 points. **Fizz** (quasi gratuit, reboucle Bellows Breath/Wages of Pain), **Star-Crossed**,
  **Watcher** (-3 à tout, casse les holds, rejouable via Arcane Shift), **Bewitching Spirit** loop
  (vide la main adverse), **Mind Splitter**, **Bellows Breath** (-might répétable). Faible vs Master Yi
  (Ruin Runner intuable), Irelia, et l'aggro (lent). **Cold Shot banni** (sinon Ezreal Prodigy + Cold Shot
  trop fort). Conseil récurrent : **savoir concéder** une game longue pour préserver le temps.
- **LeBlanc, Deceiver (Esprit/Ordre)** — la plus puissante « quand elle part », mais **trajectoire
  descendante en rounds profonds** (affronte Yi/Diana/Aurelia, tous durs). Le déclin de Gust l'affaiblit.
  Variante **Baited Hook** la plus forte. Core : **Karthus** (double chaque Death Knell), **Glass
  Mixologist** (revive ≤3 might), **Baited Hook** (cheat une grosse unité), **Mirror Image** (copie l'effet
  de carte seul), **Rune Rex + Sacrifice** (combo explosif). Cauchemar des decks de sorts de dégâts
  (Death Knell punit chaque kill). **Difficile : decks de hold + Vex** (stun les reflections).
- **Annie, Dark Child (Fureur/Chaos)** — **sous-estimée**, très bonnes conversions, plusieurs top 8
  (Prismatism). Combo de couleur solide, flexible (aggro OG Rengar ou contrôle violet). **Sneaky Deckhand**
  revient comme tech pour « aller plus vite sous l'Aurora ».
- **Azir, Emperor of the Sands (Calme/Ordre)** — non-méta sur le papier mais **vainqueur Utrecht**
  (Squirtle, invaincu) par pur skill. Go-wide token, gère bien Aurora et « protect the queen ».
  Tech : cascade d'équipements (**Soul Sword** Set 3, Eye of the Herald, BF Sword via Hall of Legends),
  triple **Arise** (briser un hold), **Death Grip** (transfert de might), double **Defy** game-ending.
  Boosté par Sprite Fountain. Pickpocket en side anti-Azir (vole ses gears).
- **Aurora (archétype gear, Body)** — **« boogeyman » du méta**, MF (Corps/Chaos) la meilleure version,
  Sivir (Corps/Chaos) gagne le miroir (meilleure rune economy via gold). **Dazzling Aurora** T3 = quasi
  imbattable. **Elder Dragon** (seuil létal ennemi → 1) + **Flurry of Blades**/**Mind Splitter** = wipe.
  **Baron Nashor** (12 might intuable + ganking). **Devient moins effrayante** au fil des RQ (sideboards
  prêts : moitié anti-Aurora). Très faible aux Légendes Ordre (Hidden Blade, Call the Weak), au gear hate
  en **unité** (Akshan, salvage), et à **Star-Crossed**.
- **Viktor, Herald of the Arcane (Esprit/Ordre)** — **montée nette via Sprite Fountain** ; « le deck le
  plus fort du jeu à 6 points ». Pur control/removal : Bellows Breath (board-wipe -1 à tout), Call the
  Weak, Imperial Decree, Wages of Pain, génération continue de recruits. Faiblesse : **Moonfall** (-2 vs
  son -1), Pickpocket.

**Tier A/B — hold & outsiders**
- **Vex, Gloomist (Calme/Chaos)** — **meilleur deck « hold » du set**, haut de B / bas de A, persistant en
  top 8. **Vex Apathetic** (stun toute unité jouée pendant le hold, ne cible pas → ignore deflect ;
  meta-defining) + pioche au hold. Évolue vers plus d'agressivité (Kha'Zix). **Sona** (ready 4 runes EOT),
  Existential Dread (stun répétable), Tiana/Ahri (rattrape le tempo de points). Difficile : **Aurora**
  (Elder assigne les létaux individuellement), boardwipes (Unchecked Power/Ruination), Ezreal contrôle.
  Erreurs de deckbuilding pointées : **ne pas jouer Grove of the God Willow** (déjà dans la Légende),
  éviter Scuttle Crab (0 might).
- **Kha'Zix, Voidreaver (Corps/Chaos)** — **réévalué à la hausse par certains** (adaptable, bon vs
  Master Yi), mais **design-flawed** selon d'autres : pas de vrai payoff XP, signature **Void Assault**
  n'est pas une « action » (pas de double-score), pas de ganking, peut bounce une unité seulement si
  exhausted. **Sous-performe** à cause de LeBlanc/Ezreal/Aurelia/Diana au top. Combo signature :
  **Irresistible Fae Folk** (traîne l'ennemi) + ambush Kha'Zix Mutating Horror. **Fizz** (rejoue ≤3),
  **Monastery of Hirana** (draw/tour). Faible vs decks de hold (Vex, Ivern).
- **Pyke, Bloodharbor Ripper (Fureur/Chaos)** — **validé top 8 Hartford** (version contrôle). Légende :
  payer 1 + tap → renvoyer une unité en main + Gold Gear exhausted (rampe). **Ripper's Bay** (rampe vers
  le **Baron Nashor**), **Bewitching Spirit** (discard la main adverse), **Star-Crossed**, **Death from
  Below** (signature kill). **Boogeyman du matchup = Vex Apathetic** qui « tue le rouge » à elle seule.
  Aggro (Lillia/Annie) difficile.

**Tier B**
- **Fiora, Grand Duelist (Corps/Ordre)** — montante (B→A) ; **Shepherd's Heirloom** (équip via XP) +
  **Kinku Initiate** (3-cost draw, MVP), bonne vs Aurora (Action/Sabotage/Weapon Master). Plan plus
  propre que Sett.
- **Sett, The Boss (Corps/Ordre)** — mur défensif (la Légende garde l'unité même quand l'adversaire
  conquiert). Moteur de valeur **Akshan + Arena's Bar** (ressuscite l'Akshan voleur de gear chaque tour).
  **Irresistible Fae Folk** (charme/drag). Nerf : il faut déjà un buff présent pour le dépenser. Souffre du
  violet (Star-Crossed) et est **trop lent** pour punir le slow-roll d'Aurora.
- **Rek'Sai, Void Burrower (Fureur/Ordre)** — **outsider Set 3** : aggro go-wide « dump la main ».
  **Void Rush** (réduit de 2, joue de n'importe où) + **Under-Titan** (2 énergie / anthem +2) + unités
  hors-main (Void Drone, Carrion Dredger) → board énorme dès T2-3. Faiblesse : coûts power élevés
  (étranglement de runes), pas d'out late vs Diana. Counter suggéré : **Repose** (anti drops hors-main).
- **Lucian, Purifier** — montant : **Blackbacks +4** (conquer garanti) + relentless/assault, un
  counterspell désormais. Bien positionné vs Aurora (orange).
- **Ornn, Fire Below the Mountain (Corps/Ordre)** — boosté par **Sprite Fountain** (résout sa lenteur),
  build aggro (Sprite Burst, Steraks, Unchecked Power).
- **Darius, Hand of Noxus (Fureur/Ordre)** — sleeper hook aggro, renforcé par les pioches jaunes ;
  **Spinning Axe** (se ré-attache), Cleave, **Rek'Sai** comme choix de deckbuilding (Draven/Darius
  accéléré). Faiblesse : économie de runes.
- **Kai'Sa, Daughter of the Void (Fureur/Esprit)** — en perte de faveur mais a les outils anti-Aurora
  (**Time Warp**, **Thermo Beam**, Singularity + Stupefy pour priver Aurora de cibles à recycler).
  Pire matchup : Irelia.
- Aussi : **Yasuo** (couleurs top mais support set va vers le hold ; Treasure Hunter → green spells payés
  sans power, jusqu'à 5 Defy/tour ; punit Vex en mobilité), **Jax** (protect-the-queen gear-stack),
  **Volibear** (build dragons/Elder, high-rolls > Viktor), **Ivern** (hold « spammy », Brush symétrique,
  mauvais vs Draven).

**Tier C / situationnel**
- **Rengar, Pridestalker (Fureur/Corps)** — **épitomé du C** (high-roll), aggro Ambush ultra-rapide,
  quasi pas de top end. Score au tour adverse via **Thrill of the Hunt**. **Crushé par Vex Apathetic**.
  A néanmoins fait **2e Vancouver** (Sam) = plafond élevé pour un excellent pilote.
- **Lillia, Bashful Bloom (Esprit/Calme)** — jugée overhyped (mid-C) mais reste un tempo ultra-rapide
  (spam de Sprites, jamais de hold, vise 6 points vite). **Sprite Fountain** insane. Pire matchup :
  **Vex Apathetic** + aggro-hold (Draven). Plafond limité (la Légende wipe son propre board en début de tour).
- **Lux, Lady of Luminosity** — **deck loop (Echo + Sacrifice + Forge of the Future)** = ressources
  quasi-infinies, top 8 Hartford. Unanimement jugé **malsain** (« glitch » non intentionnel). Contre-jeu
  Master Yi : Sabotage (vole Progress Day), Ruination (reset), Charm sur le champion Lux, pression rapide.
- **Jhin, Virtuoso (Fureur/Esprit)** — « tier 4 » en blague mais a un plan : 4 sorts ≥4 énergie → channel
  4 runes + draw. Base removal massif (Curtain Call, Deadly Flourish, Singularity, Time Warp). Bon vs les
  decks qui développent un board (Rengar/Diana/Draven). Faible vs vert/Deflect, Vex, Master Yi XP.
- **Leona (F)** — Vex fait mieux le stun ; **Renata (F)**, **Jinx (D)**, **Ahri (D)**, **Poppy/Garen**
  (mauvaises Aurora jaunes sans dig).
- **Master Yi, Wuju Master (Corps/Calme)** — variante XP de niche (≠ Bladesman). Hunt 2 via
  **Master Yi Tempered**, paliers niveau 6 (+1 might à tout) et 11 (tout entre ready). Payoffs : Elder
  Dragon, White Flame Protector, Master Yi Unstoppable. Aucun avantage early (handicap dans un méta tempo).

## 3. Matchups clés (favori → pourquoi)

- **Master Yi > Ezreal / Pyke / Viktor / Rengar / Vi** : Ruin Runner intuable, Trophy Hunter, Punch First ;
  Viktor renverse sur God-start.
- **Diana > Master Yi** (serré) : Hway + draw, Baron Nashor side (3e battlefield casse les maths) ; MAIS
  Yi favori avec Ruin Runner main (3x) + gear hate. Finale Tianjin : **Yi 2-1**.
- **Diana > Vex / Annie / Rek'Sai / LeBlanc / Lillia** : pure valeur (Hway, Moonfall, Vex Apathetic,
  Star-Cross anti Rune Rex). Annie tente Vex Cheerless + Invert Timelines ; Rockfall Path interdit l'ambush.
- **Irelia vs Aurora** : **Irelia favori SI elle tech** (Adaptatron/Rake anti-gear on-play, Star-Cross/Mind
  Splitter vs Elder, Defy). Sinon Aurora high-roll gagne.
- **Irelia > Vex** : Vex trop lent à s'établir ; Abandoned Hall double-dip avec Aurelia Fervent.
- **Azir > LeBlanc / Sett / Viktor (finale G3)** : nier le 1er conquer ; cascade d'équipements + Arise +
  double Defy. Peut étirer le timer pour forcer le tie G3 contre LeBlanc lente.
- **LeBlanc > Kha'Zix** : high-roll Dustgrub Lab + Karthus intuable ; Kha'Zix manque d'outils.
- **Ezreal > Pyke** (contrôle, voit plus de cartes) ; **Kha'Zix > Teemo** (Monastery draw).
- **Rek'Sai > Fiora / Irelia** (tempo go-wide) ; **Draven ~ Irelia** (~50 %), **Draven < Aurora**,
  **Draven détruit par Ezreal**, **Draven > Ivern** (anti-hold).
- **Kai'Sa > MF Aurora** (Time Warp + priver de cibles à recycler) — matchup réputé défavorable, gagné au skill.
- **Jhin > LeBlanc** (removal efficient + Time Warp), matchup pourtant défavorable.
- **Sett < Aurora** (trop lent), **Sett > Jinx** (anti-aggro), **Rumble** peut surprendre LeBlanc (mur de might).

## 4. Cartes / tech qui définissent le format

- **Vex Apathetic / Vex Cheerless** — Apathetic stun l'ambush et tout ce qui « arrive » (anti
  Rengar/Annie/Master Yi/Lillia/LeBlanc) ; Cheerless taxe énergie+power à chaque showdown (anti
  Irelia/Diana/Master Yi). Ne cible pas → ignore deflect.
- **Defy** — ~40 % des decks, counter ~70 % des sorts ; la tech verte/jaune la plus « game-ending » en top
  cut. Toujours menacée par « 1 rune ouverte ». À garder pour Moonfall/Star-Cross/Arise, jamais sur un Gust.
- **Star-Crossed (+ Fizz)** — pivot du chaos : bounce action-speed (les DEUX unités), fizzle Baited Hook,
  reset Set/Fiora/Renata ; Fizz le reboucle. Némésis n°1 des Elder Dragons et d'Aurora/Aurelia/LeBlanc.
- **Moonfall** (Diana) — -2 à tout le battlefield, action-speed, meilleur signature du jeu (scaling -4/-8).
- **Ruin Runner** — mur intuable de Master Yi ; **mais meurt à Flurry of Blades** (qui ne cible pas).
- **Rengar Trophy Hunter** — ambush surprise (champion zone), carte différenciante de Master Yi.
- **Baron Nashor** — 12 might intuable + ganking (anti-Aurora) ; peut entrer dans n'importe quel
  battlefield via le Baron Pit (souvent oublié).
- **Hway/Hui** (Diana), **Scuttle Crab** (29 % Hartford, draw + info ; mauvais en Vex), **Tide Turner**
  (loop de valeur ; sert le 8e point de Diana), **Sona** (Vex, ready 4 runes EOT).
- **Adaptatron / Disarming Rake / Action / Akshan / Sabotage / Thermo Beam** — réponses anti-Aurora
  (gear hate, surtout en **unité** que Sabotage ne recycle pas).
- **Fiora Peerless** (Master Yi), **Acceptable Losses** (Diana anti-gear), **Heedless Resurrection** (miroir
  Aurora), **Soul Sword / Sprite Fountain** (gears Set 3 qui boostent Azir/Viktor/Ornn).

## 5. Surveillance ban (avis casters, NON officiel)

- **Vex Apathetic** — le plus discuté : floodgate **sans coût en power**, force le de-ramp adverse,
  ~26 % des decks (≈100 % des decks violets). « Défaut de design » pour certains.
- **Star-Crossed** — bounce action-speed pouvant cibler la base ; les casters voudraient le limiter aux
  battlefields.
- **Echo** (Set 1) — active le **loop Lux** (ressources infinies), unanimement jugé malsain.
- **Ferris Forerunner** — polarisant (mech tokens, gros might) plus que « broken ».
- Déjà bannie : **Cold Shot**.

## 6. Règles & format (mises à jour à connaître)

- **Bo1 : sideboard AVANT la partie** désormais autorisé (façon Flesh and Blood) → tout le monde
  side-in Aurora ; piège pour qui choisit Sivir/MF en mid-range (pas de gear).
- **Nerf des buffs (touche Sett)** : on ne peut plus « conquer non-buffé puis dépenser le buff » ; il faut
  déjà avoir un buff. L'ordre de layering compte.
- **Structure des triggers** : une fois le coût payé et l'effet sur la chain, le trigger est **verrouillé**
  (résout même si la condition devient fausse — ex. Sona, Loose Cannon). « You may » dans la **condition** =
  on décide de le mettre sur la chain (refus = pas de fenêtre de réaction adverse) ; « may » à la
  **résolution** = l'effet va sur la chain, on choisit en résolvant.
- **Battlefields contestés** : une unité qui entre sur un battlefield non-contesté le rend contesté →
  **showdown immédiat**. Mais tant qu'un effet (Charm) est sur la chain non résolu, le contrôle NE change
  PAS (le showdown attend la résolution).
- **Emperor's Dias (ruling)** : on peut **refuser de payer le 1 énergie** à la résolution (« may pay one
  energy AND », pas « to ») → on renvoie l'unité en main **sans** le token Sand Soldier. Jugé non
  intentionnel → errata probable (pas un ban).
- **Elder Dragon** : seuil létal des ennemis ramené à **1** ; le passif tue **rétroactivement** les unités
  déjà endommagées (Flurry of Blades avant Aurora tue plus).
- **Star-Crossed** : retourne les DEUX unités (amie + ennemie), « do as much as you can ».
- **Mirror Image** (LeBlanc) : copie **seulement l'effet de carte** (ni « on play », ni malus de
  battlefield) ; le Deceiver crée un token reflection 0/0 puis copie les stats (pas de Zillion infini).
- **Ivern + Brush** : le remplacement track toujours le battlefield original → on ne peut pas « brush un
  brush » pour le supprimer. **Ivern + Aspirant's Climb** à 8-8 : pas de fin instantanée, victoire au
  prochain point « normal ».
- **Combat « no result »** (pas de pioche/score) si recall au cleanup, ou les deux ont encore des unités,
  ou aucun n'en a (ex. Leona qui stun puis doit recall → le Draven ne pioche pas).
- **Trois sens de « play »** : résoudre (un sort defied n'est PAS « joué » pour Darius/Ravenbloom) /
  finaliser sur la chain (un sort countered compte pour Legion) / placer+filer (les discounts type Here to
  Help s'appliquent avant finalisation).
- **Signature spells / Bullet Time** : 1 énergie pour mettre sur la chain (defiable), **le power
  seulement à la résolution**.
- **Overtime (Swiss)** : il faut gagner **par 2** → un score 6-7 peut finir en **TIE** (les deux ratent le
  top cut). **Top cut = untimed**. Conseils : matérialiser les triggers (dés/compteurs de might surtout pour
  les decks hold), scooper tôt une game perdue, jouer vite.
- **Take-back** toléré à haut niveau tant qu'aucune information n'a été révélée (ruling Lille).
- **Max champions par Légende = 4** (Master Yi), sinon 3.
- **Errata attendue** : Stalking Wolf (Ivern) — le wording actuel ne matche pas l'intent dev.

## 7. Facteur humain (top cut)

Plusieurs séries décidées par la **fatigue** (finales « on-time », 15e-16e game, ~7 h) : math ratées
(kill manqué à Utrecht), blunders (Rengar à Xi'an : re-Akshan sans lire le texte, oublis de buffs). Le
trigger **Arena's Greatest** (point gratuit, raccourcit à 6) est souvent oublié et débattu comme
« oppressif » contre l'aggro (retire un tour).

---

*Incertitudes globales : noms de cartes en transcription audio approximative à vérifier sur le scrape brut
avant tout seed (ex. « Stellacorn/Silcoon Herd », « Mr. Root », « Brin here »). Certains taux de conversion
sont donnés « de mémoire » par les casters. Les opinions de tier sont éditoriales, pas data-backed.*
