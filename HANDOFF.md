...
## Session du 29 août 2026 — module bulking : renommage, phases 1 à 5, tout en local

Travail local non poussé. Aucune base de production touchée, aucun schéma appliqué ailleurs qu'en local.

**Le module ne s'appelle plus UwUTCG.** Il s'appelle « bulking » : routes `/admin/bulking` et
`/api/admin/bulking`, fichiers `src/lib/bulking-*.ts`, modèles Prisma préfixés `Bulk`
(`BulkInventory`, `BulkIntake`, `BulkProductRecipe`...). Le mot UwUTCG n'existe plus dans le dépôt.
Les tables locales ont été renommées en SQL (`ALTER TABLE ... RENAME`) avant `prisma db push`, donc
les langues, emplacements et brouillons de la veille sont intacts. **En production, ces tables
n'existent pas encore : rien à migrer, le premier `db push` créera directement les noms Bulk.**

Ce que la session ajoute, par-dessus les phases 1 et 2 déjà écrites la veille :

- **Phase 3, consultation.** `/admin/bulking/inventory` a ses filtres dans l'URL (recherche, set,
  langue, emplacement, manquants ou réservés), sa pagination serveur de 100 lignes, et une boîte
  d'action par ligne pour corriger ou transférer. Le registre a sa page à part,
  `/admin/bulking/movements`, en lecture seule et filtrable. Le tableau de bord affiche quatre
  chiffres lus en direct.
- **Deux routes d'écriture** : `POST /api/admin/bulking/inventory/adjust` et `.../transfer`. Elles
  passent toutes les deux par `appliquerMouvement`, dans une transaction `Serializable`. Aucune route
  du dépôt n'écrit une quantité sans mouvement.
- **Phase 4, recettes.** `src/lib/bulking-recipes.ts` porte le calcul pur `analyserRecette` : les
  emplacements s'additionnent, les langues restent séparées, le coût reste en `Decimal`. Trois routes
  et trois écrans (`/admin/bulking/recipes`, `/new`, `/[id]`).
- **Phase 5, decks officiels.** `src/lib/bulking-decks.ts` convertit un deck en exigences (réserve
  exclue par défaut), deux routes d'analyse et de création, un écran `/admin/bulking/decks/[id]`, et
  un lien « Stock Bulking » dans les deux tables de `/admin/decks`.

**Piège déjà payé :** la fiche recette avait recopié le calcul de disponibilité au lieu d'importer
`analyserRecette`. Corrigé. Une règle métier s'importe, elle ne se recopie pas, sinon les deux
copies divergent en silence.

Preuves du dernier passage, codes de sortie contrôlés : `npx next build` 0, `npx tsc --noEmit` 0,
`npm test` 66 fichiers et 378 tests verts, `npm run lint` 0 erreur et 99 avertissements.

Essai de bout en bout contre les vraies routes, base locale, avec le cookie admin :

- un lot dont le compte ne colle pas est refusé (« Le lot contient 3 cartes sur 1 annoncées ») ;
- un lot de 4 cartes à 40 € se comptabilise, coût unitaire 10 € réparti, 2 mouvements créés ;
- le comptabiliser une seconde fois rend 409 ;
- une correction de +2 à 20 € porte le coût moyen à 14 €, un transfert reporte ce coût, un transfert
  plus grand que le stock rend 409 ;
- l'analyse d'une recette rend 1 réalisable pour 38 € de stock, la carte limitante est signalée ;
- sans session, l'écriture rend 401.

**La base locale garde ces données d'essai** (un lot « Essai d'acceptation », une recette « Essai
acceptation », trois lignes de stock). Elles ne gênent rien, mais ce ne sont pas de vraies cartes
achetées : les supprimer demande de supprimer aussi leurs mouvements, ce que le registre interdit
par principe. Repartir d'une base propre si le premier vrai lot doit être seul.

**Reste à faire :** la tâche 13 du plan, c'est-à-dire le déploiement. Dans l'ordre : `prisma db push`
sur la base de production dans une fenêtre convenue, déploiement du code, puis seulement après,
ajout des tables Bulking à `TABLES_ATTENDUES` de `migrate-schema.mjs` dans un second passage. Ne pas
comptabiliser un vrai lot en production sans l'accord d'Allan.

Fichiers d'entrée : `docs/superpowers/plans/2026-08-29-bulking-inventory.md` (le plan complet),
`src/lib/bulking-stock.ts` (le seul passage d'écriture du stock), `src/lib/bulking-recipes.ts`.
